...
if [catch {
# entrydict label value length dict
proc entrydict {a b c d} {
	if [dict exists $d $b] {
		entry	$a [dict get $d $b] $c [expr [pos]-$c]
	} else {
		entry	$a $b $c [expr [pos]-$c]
	}
}
...
proc readTIHex {} {
	section "TI-Hex section" {
		...
		entrydict	"Field type" $type 2 [dict create 0 Data 1 End 2 Page]
...
requires	0 "2A 2A 54 49 46 4C 2A 2A"

ascii	8 "Header"
section	"File packaging" {
	entry	"Version" "[format %x [uint8]].[format %x [uint8]]" 2 [expr [pos]-2]
	set	binary [hex 1]
	entrydict	"Binary flag" $binary 1 [dict create 0x00 "Binary" 0x01 "INTEL"]
	entrydict	"Object type" [hex 1] 1 [dict create 0x00 "!Z80" 0x88 "Z80"]
	hex	4 "Date"
	entry	"Name" "[uint8],[ascii 8]" 9 [expr [pos]-9]
	hex	23 "NULLs"
	entrydict	"Calc type" [hex 1] 1 [dict create 0x73 "TI-8\[34] Plus" 0x74 "TI-73" 0x88 "TI-92 Plus" 0x98 "TI-89"]
	entrydict	"Variable type" [hex 1] 1 [dict create 0x23 "OS" 0x24 "APP" 0x25 "Cert" 0x3E "License"]
	hex	23 "NULLs"
	entrydict	"Prod ID" [hex 1] 1 $ProdIDs
	set	datasize [uint32]
